...
```
 ...
 O DFFITS recomenda a investigação dos pontos 90 e 71, mas o ponto 90 realmente parece não ser anormal, para o ponto 71 é interessante saber o conhecimento prévio da área para referendar ou não sua exclusão. Para o gráfico do CovRatio tivemos:
 
```{r, echo = F,message=FALSE}
#install.packages("olsrr")

plot(covratio(modelo))

covratio(modelo)[covratio(modelo)<1]

```
 Com a observação 90 diminuindo a precisão das estimativas dos parâmetros, apesar de haver outras observações mas que estão muito próximas e até podem ser arredondadas para 1, a observação 90 de fato, parece não ser implausível, a menos que o problema esteja no valor em dólares cobrado em seu seguro saúde (USD 62.592,87) que parece estar super-faturado se comparado a observação 91 de um indivíduo de 42 anos e obesidade mórbida, para realizar análises em relação ao valor cobrado no plano de saúde destes clientes é interessante saber o motivo do valor cobrado e se realmente são plausíveis, cabe também investigar a observação 71 do indivíduo de 22 anos a quem foi cobrado USD 44.501 e teve um IMC>50 registrado. 
