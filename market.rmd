...
## Modelo com o método Forward
  ...
  O modelo com o método *Forward* foi de forma $$\begin{split} Sales_i=-0,10+3,56\cdot TV_i-0,004\cdot Radio_i+0,004\cdot Social.Media_i+  \\ 0,01\cdot InfluencerMega_i-0,05\cdot InfluencerMicro-0,07\cdot InfluencerNano_i \end{split}$$, ou seja caso o investimento em TV seja de 1 milhão de unidades monetárias (U.M.), não seja feito investimento em rádio, e o investimento em mídias sociais seja de 100 milhões com a colaboração de um Mega Influencer espera-se que em média o valor das vendas seja de 7,47 milhões U.M., para todos os outros tipos de influencers há de se esperar que haja um efeito negativo sobre o número de vendas, sendo melhor investir no Mega Influencer. Além disso um efeito negativo sobre as vendas ocorre ao se investir em propagandas no rádio, sendo interessante evitar este meio de divulgação e focar em outros que possam trazer mais retorno.
...
Por fim o gráfico de Resíduos x Leverage nos indica possívels casos de muita influência no modelo, i.e outliers, pontos que podem influenciar muito a análise e talvez devam ser excluídos. Assim, podemos observar a presença de 3 possíveis observações que podem ser outliers. É importante observar que essas observações também são sinalizadas nos outros gráficos da figura. Iremos estudar o que fazer com essas observações nos próximos relatórios.
...
### Figura 7 - Análise de Resíduos - Backward/Stepwise.

Da mesma forma como no modelo anterior temos que:

Através da análise de diagnóstico do modelo linear múltiplo ajustado, primeiramente podemos observar que na Figura 7, o gráfico de Resíduos x Valores ajustados não nos mostra nenhum padrão de dispersão dos pontos e a linha  vermelha no meio do gráfico, que deve ser aproximadamente horizontal no zero, também segue esse comportamento. Logo, juntamente com o gráfico Quantil-Quantil, podemos afirmar que os pressupostos de normalidade e linearidade dos resíduos são cumpridos.
